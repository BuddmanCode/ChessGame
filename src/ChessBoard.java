public class ChessBoard {
    ...
    public boolean notOwn(int toLine, int toColumn, String color){
        if (!isEmpty(toLine,toColumn))
            if(this.board[toLine][toColumn].getColor().equals(color))
                return false;
        return true;
    }

    public boolean isEnemy(int toLine, int toColumn, String color){
        if (!isEmpty(toLine,toColumn))
            if(!this.board[toLine][toColumn].getColor().equals(color))
                return true;
        return false;
    ...
    public boolean isEmpty(int toLine, int toColumn){
        return this.board[toLine][toColumn] == null;
    }
    public boolean inBoard(int toLine, int toColumn) {
    ...
    public boolean moveToPosition(int startLine, int startColumn, int endLine, int endColumn) {
        if (checkPos(startLine) && checkPos(startColumn)) {
            ...
            if (board[startLine][startColumn].canMoveToPosition(this, startLine, startColumn, endLine, endColumn)) {
                ...
                if(board[endLine][endColumn].getSymbol().equals("P") && (endLine == 0 || endLine == 7))
